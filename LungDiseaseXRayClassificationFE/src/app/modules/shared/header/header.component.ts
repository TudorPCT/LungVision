...
export class HeaderComponent implements OnInit{
  ...
  getLanguages() {
    ...
    let supportedLanguages: string[] = this.translate.getLangs();

    supportedLanguages.forEach((lang: string) => {
      const key = lang as keyof typeof isoLanguages;
      if (isoLanguages[key]) {
        languages.push({name: isoLanguages[key].name.split(", ")[0], code: lang});
      }
    });
    return languages;
  }
